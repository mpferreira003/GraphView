...
cosine_f = lambda u,v: np.sum(u*v)/(sqrt(np.sum(u**2)) * sqrt(np.sum(v**2)))
euclidian_f = lambda u,v: sqrt(np.sum((u-v)**2))
metrics = {
...
class MundoPequeno():
    ...
    def create_data(self,dim:int=2,space:int=1,metric:str='euclidean',verbose=False):
        ...
        for i in range(self.n):
            ...
            if verbose: print(f"Calculating distances: {100 * float(i) / self.n:.2f}%")
        ...
        self.organized_nearest = self.organized_nearest[:,1:] ## dropa o primeira conexão (ele mesmo), ficando uma matriz NxN-1
    ...
    def create_connections(self,k:int,p:int,verbose=False):
        ...
        if not self.has_data:
            raise ValueError("Erro - você deve chamar o .create_data para gerar os dados e as distâncias")
        ...
        if verbose: print(f' os nós p_choosed_nodes[{p_quantity}] terão conexão com um nó distante: {p_choosed_nodes}')
        ...
        if verbose: print(f'far_neighboor: {far_neighboors}')
        
        for node,neighboor in list(zip(p_choosed_nodes,far_neighboors)):
            distance = self.distances[node][neighboor]
            self.connections.append([int(neighboor),int(node),float(distance)])
            self.connections.append([int(node),int(neighboor),float(distance)])
